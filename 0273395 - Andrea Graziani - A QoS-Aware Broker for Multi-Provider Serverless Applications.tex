...
\begin{frontespizio} 
	...
	\Logo[3cm]{./Images/UniLogo/LogoTorVergata.png}
...
\begin{center}
	\textit{Dedicated to my beloved A.}
\end{center}
...
\section{Serverless Choreography Structure}
...
In this section, we will describe and analyze both approaches, although only for the latter we have developed and implemented a heuristic approach capable to solve it in a faster way with respect to the exact algorithm. 
...
Therefore we delay pheromone trails update until the first time it is needed by an ant, allowing our algorithm to perform faster. 
...
\begin{algorithm}\label{antsolutiongen}
	...
	\For{$l\gets0$ \KwTo $m$ \KwBy $1$}{
		...
		$o_{i_j} \leftarrow$ Randomly select an object from $\textbf{G}_i$ which does not violate resource constraints\;
		...
		\While{$\textbf{G} \neq \emptyset$}{
			...
			$\mathcal{O} \leftarrow $ Select a group of candidates objects belonging to $\textbf{G}_a$ which do not violate resource constraints \;
...
According to \citet{acosemplice} research, the selected parameters allow our algorithm to quickly find good solutions even though it may fail to find the optimal (or best) solution. The number of ants is set to $4$, which is the double of the number of physical cores in our CPU. Finally, we limited the number of iterations of our algorithm to $1000$.
...
\begin{figure}[h]
	...
	\end{tikzpicture}
	\caption{The image-processing serverless choreography used for our experiments.}
...
\begin{figure}[!ht]
	...
	\begin{tikzpicture}
		...
		\matrix [xshift=-1.5cm] at (current bounding box.south east) {
			...
			\node[circle,minimum width=0.2cm, right of=b, xshift=0.65cm, yshift=-0.05cm] {\ItalicQuotMark{OpenWhisk 2}}; \\
		};
