...
Any solution considers the existence of multiple implementations, or versions, of a same serverless function, that usually exhibit different performances and cost. Currently, only \citet{NUOVODOCUMENTO}'s solution supports serverless functions invocations on multiple FaaS provider platforms which is exploited to guarantee the respect of user specified QoS constraints.
...
\begin{table}
	...
	\begin{tabular}{l|l}
		...
		\texttt{aws/aws-sdk-go-v2}\footnote{\url{https://github.com/aws/aws-sdk-go-v2}} & AWS SDK for the Go programming language \\
...
At the end of our software development process, which ends with a software made up of $8405$ lines of code, SonarQube reports $0$ code smells, $0$ duplicated code blocks and $0$ security vulnerabilities. This result should ensure an high software maintainability, simplifying the development of future release.
...
As TSDB, that is for the storage and the retrieval of time series data regarding serverless function performance, we adopted InfluxDB\cite{influx}, an open-source time series database written in the Go programming language.
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item $P : \Phi \times \Phi \times \Rplus \to \Rplus \cap \left[ 0, 1 \right]$, called \textit{transition probability function}, is such that $P\left(\phi_i, \phi_j, t \right) = p_{ij}$, where $t$ is the time. Obviously, when $P\left(\phi_i, \phi_j, t \right) = 0$ or, similarly, when $\left( \phi_i, \phi_j \right) \notin E$, $\phi_i$ cannot invoke $\phi_j$.
...
Referring to sub-choreographies, we will use $P_{exit}(\mathcal{C}^*,t)$ to denote the so-called \textit{sub-choreographies exit probability}, that is the probability to ends the execution of $\mathcal{C}$ when $\mathcal{C}^*$ is executed; later, we will explain how to compute it.
...
Form now, we will use $E[I_{\theta}]$ for $\theta \in \Theta_{\mathcal{C}^*}$ to denotes the expected value of the number of invocation of the choreography $\theta$.
...
Moreover, it is possible to compute the exit probability $P_{exit}(\theta,t)$ of the sub-choreography $\theta$ of $\mathcal{C}$, using following formula:


\begin{equation}
	P_{exit}(\theta,t) \mathDef \sum_{i = 1}^n TPP(\pi_i) \cdot P_{exit}(\mathcal{E}_i, t)
\end{equation}

where:

\begin{itemize}
	\item $n$ is the number of exit structures contained inside $\theta$.
	\item $\pi_i$ is the path starting from the entry point of $\theta$ to the $i$-th exit structure, for $i \in \SetFromOneTo{n}$.
\end{itemize}
...
Although firstly each ants selects randomly an object of his partial solution from a randomly chosen group, all subsequent objects are selected according to so-called \ItalicQuotMark{transition probability}, which depends on several parameters:
...
represents the tightness of the object $o_{i_j}$ on the problem constraints relatively to the constructed solution $\textbf{S}_{k_l}^{(z)}$; thus, the lower this ratio is, the
