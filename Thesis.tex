...
One of our framework's goal is to exploit its self-adaptation capability to fulfill \textit{non-functional SLA requirements}, considering the \textit{average values} of the following attributes:
...
According to our model, let $u$ an user, a SLA can be defined as follows:

\begin{equation}
	SLA_u \mathDef \left\langle (RT,w_{RT}),(C,w_{C}) \right\rangle 
\end{equation}
...
\begin{itemize}
	...
	\item $C \in \mathbb{R}^+$ is the upper bound on average service cost per invocation.
	
	\item $w_{RT}$ and $w_{C}$ are weights for the different SLAs attributes such that:
...
The goal of our framework is to guarantee guarantees that thresholds $RT$ and $C$ will hold on average. Informally, to meet aforementioned objective, our framework have to identify a suitable configuration when it have to execute an serverless application.
