...
\section{Function instances states}
...
The serverless computing paradigm has attracted a great deal of interest from both academia and industry [8], [19],
[40]. However, few research studies have been directed at
the modeling and optimization of performance and cost of
serverless applications. Therefore, we provide an extensive
review of the literature in three fields, which are inherently
related to our topic, namely the performance modeling
of cloud applications, workflow scheduling in the cloud
environment, and serverless architecture.
...
We developed our system from scratch using Go programming language\cite{Go} and the external libraries reported in table \ref{tab:libraries}. 
...
Code smells are neither bug nor technically incorrect events and do not prevent the program from its expected behavior, however scientific literature shows that a correlation between code smells and the risk of bugs exists; in fact, the greater the number of code smells, the greater the risk of bugs within the code. 

Moreover, code smells can be an indicator of factors that contribute to so-called \textit{technical debt}. We call technical debt the result of past decisions from software development point of view that negatively affect the project future. Steve McConnell defined technical debt as  \QuoteIntro \textit{A design or construction approach that's expedient in the short-term but that creates a technical context in which the same work will cost more to do later than it would cost to do now}". In other words, technical debt regards any quality problem in the source code like duplication, excessive complexity and design problems.

When technical debt grows at an uncontrollable rate, you can be rapidly reach a point making the software evolution unfeasible, requiring a complete software refactoring. To overcome such situation, it is important that the technical debt is properly managed and kept under control during software development.
