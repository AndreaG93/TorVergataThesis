...
Despite it is very easy to implement, the adopted centralized approach also has, obviously, several shortcomings since it is both not scalable and and network traffic inefficient, exacerbating traffic congestion problems, beside from representing a single point of failure \cite{SDCC}. 
...
Our framework consists of following main logical entities:

\begin{description}
	
	\item[Orchestrator] This is the most important software component of our system whose aim to allow the composition of serverless functions in order to properly execute applications, defining a stateful serverless workflows. 
	...
	More concretely, the orchestrator is invoked when the execution of any serverless application is required by an user, passing a JSON input which represent, obviously, the input data for the invoked application; then, when the execution is ended, it will be generated a JSON output which will be sent to the client.
	
	Each function's invocation of any serverless workflows is performed synchronously, which means that the orchestrator waits for the execution of the orchestrated functions to complete before to attempting to invoke the next one. Unfortunately, at the current state of our design, if any error occurred during the execution of any orchestrated functions, the entire application's execution will be aborted. When the execution of any orchestrated function ends, the orchestrator retrieves log information, mostly regarding performances and billed costs, from live metric streams from FaaS provider logs, which are forwarder to our logging subsystem. 
	...
	Actually, our orchestrator subsystem supports \textit{function chaining} and \textit{nesting} exploiting several control-flow structures in order to allow:
	
	\begin{itemize}
		\item \textit{functions branching}, through the support for \texttt{if} and \texttt{switch} statements.
		\item \textit{functions looping}, according to which the use of both \texttt{for} and \texttt{while} statements are supported.
	\end{itemize}
	
	Moreover, the orchestrator is capable to manage \textit{parallel executions}. It is also possible to \textit{abort} current application's execution exploiting the \texttt{exit} statement.
	...
	To overcome aforementioned limitation, our orchestrator is able to select a specific implementation of a functions inside any serverless workflow; concretely, it can run a function that implements the Pollard's rho algorithm for integer factorization, written in Python, instead of a function implementing the trial division based algorithm, written in JAVA. Moreover, it can invoke functions hosted on multiple FaaS platforms, which can very useful in some situations.
	
	However, only our QoS-aware model-bases optimizer has the responsibility to determine which implementation, for each function belonging to an application, must be invoked in order to meet QoS level constraints. To be more precise, during function orchestration, the orchestrator limits itself to invoke function implementations according to the so-called \textit{choreography configuration}, or simply \textit{configuration}, produced by the optimizer. The choreography configuration tells to the orchestrator which implementation and configuration must be used for each function in order to meet user specified QoS level constraints; we will talk more in detail about this shortly.
	
	\item[QoS-aware model-bases optimizer] As already said, the optimizer has the responsibility to identify, for each serverless function belonging to an application, which configuration and implementation must be used in order to satisfy user specified constraints.
	...
	\begin{itemize}
		...
		\item if QoS level constraints are specified, the optimizer firstly  retrieving, interacting mostly with our logging subsytem, all needed metrics to perform its task, such as observed average response times and charged cost for each implementation and configuration for every function inside the invoked applications; moreover, the optimizer will also checks both the status for each FaaS platforms and the actual users requests traffics on the system. We will precise what metrics are effectively used by the optimizer in next chapter.
		
		When all needed data are collected, the optimizer provides a suitable configuration of each serverless function within an application, by solving an optimization problem, derived from our performance model. 
		
	\end{itemize}
	
	When a configuration is finally produced, the orchestrator will be subsequently invoked with it to actually perform the execution of the requested application.
...
In order that our optimizer performs its tasks, a live overview of the state of both all FaaS platforms executions environment and the system is, obviously, required, including a dataset containing all historical information regarding serverless function performances under any possible configuration (in term of RAM memory size values).

As known, anytime a serverless function belonging to a given application is invoked by the orchestrator, the FaaS platform provider, which hosts aforementioned function, produce a log containing several kind of information as:

\begin{itemize}
	\item Time spent to complete the serverless function's execution.
	\item Charged cost.
	\item RAM memory size value.
	\item The occurrence, or not, of a cold start.
\end{itemize}

In order to build our aforementioned dataset, all FaaS produced logs are continuously retrieved, parsed and stored by the logging subsystem during the entire life-cycle of our system.  

FaaS platforms logs data \textit{can}\footnote{Strictly speaking they aren't.} be considered as time series data, that is data points indexed in time order. Each data point following attributes:





 according to which each data point  the time of each instance, containing the attribute value regarding power consumption, is given by a timestamp attribute; thus our data-set represents a sequence of discrete-
time data 



From an architectural point of view, the logging subsystem is made up of three main logical components:
...
As the Logging Framework receives logs from each of
its sources, it computes running metrics such as total con-
currency of specic functions and chains, number of drops
observed in the serverless framework, and inter-arrival times
of function chains tracked by the Producer. These metrics
are available to be read at any time by the Policy Framework
and can be exported to long-term storage to be available for
post-hoc analysis.
...
%	\item[Parser] This is the component
...
\begin{description}
	...
	\begin{itemize}
		...
		\item If the JSON's representation of the serverless application to create contains syntax errors, the code \texttt{400} (Bad Request) will be sent. That status code, will be sent also when a malformed URI is submitted, that is when the user \texttt{genericOwner} does not exist.
